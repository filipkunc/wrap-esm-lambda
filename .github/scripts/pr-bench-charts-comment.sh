...

{
  ...
  echo
  echo "The committed reference charts at $HEAD_SHA — the set the release page embeds. This run reports the base-vs-head cold-start table, transform diagnostics and the same-fixture Orchestrion comparison table in the [run summary]($RUN_URL). The bench-charts artifact on the same page includes the freshly rendered Orchestrion comparison chart, its Markdown table and raw JSON."
  echo
  ...
  echo
  echo "**Wrap vs Orchestrion** (same fixture, Promise path, different mechanisms): see the table in the [run summary]($RUN_URL) and orchestrionComparison.svg in the bench-charts artifact."
} > "$body_file"
