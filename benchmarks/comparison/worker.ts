...
assert.ok(
  tool === "wrap-esm-lambda" || tool === "orchestrion",
  "expected a comparison tool argument",
);
...
const smithyPath = smithyPackagePath.replace(
  /package\.json$/,
...
const orchestrionTransformer = matcher.getTransformer(
  "@smithy/core",
  smithyPackage.version,
  filePath,
);
assert.ok(
  orchestrionTransformer,
  "Orchestrion did not select the Smithy target",
);
...
const transform =
  tool === "wrap-esm-lambda" ? transformWithWrap : transformWithOrchestrion;
const verified = transform();
assert.notStrictEqual(verified, source, `${tool} returned the input unchanged`);
if (tool === "wrap-esm-lambda") {
  assert.match(
    verified,
    /@wrap-esm-lambda instrumented/,
    "Wrap output is missing its transform sentinel",
  );
} else {
  assert.match(
    verified,
    /diagnostics_channel/,
    "Orchestrion output is missing its channel integration",
  );
  assert.match(
    verified,
    /smithy-send/,
    "Orchestrion output is missing the configured channel",
  );
}
...
assert.ok(
  ...
  "BENCH_COMPARE_TIME_MS must be positive",
);
...
const orchestrionPackage =
  require("@apm-js-collab/code-transformer/package.json") as {
    version: string;
  };
