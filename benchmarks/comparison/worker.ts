...
const smithyPackage = require('@smithy/core/package.json') as { version: string }
...
const wrapEntries: InstrumentEntry[] = [
  {
    module: { name: '@smithy/core', versionRange: smithyPackage.version, files: [filePath] },
    patch: { name: 'patchClientSend', from: '/benchmarks/comparison/patch.mjs' },
    bindings: ['Client'],
  },
]
...
const { create } = require('@apm-js-collab/code-transformer') as {
  create(config: unknown[]): {
    getTransformer(moduleName: string, version: string, path: string): {
...
const matcher = create([
  {
    ...
    module: { name: '@smithy/core', versionRange: smithyPackage.version, filePath },
