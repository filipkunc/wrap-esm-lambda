...
export interface BenchTarget extends SurfaceTarget {
  minMs: number
  iterations: number
  tier: 'append' | 'rewrite'
  bytesIn: number
...
const measured: BenchTarget[] = targets.map((target) => {
  ...
  const opts = { format: target.format, delivery: 'registry' } as const
  ...
  return {
    ...
    hash: createHash('sha256').update(applied.code).digest('hex'),
    mapHash: applied.map == null ? null : createHash('sha256').update(applied.map).digest('hex'),
  }
})
