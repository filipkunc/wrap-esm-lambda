...
import * as engine from '@wrap-esm-lambda/engine-acorn'
import { isMissingExportError } from '@wrap-esm-lambda/core'
import { captureThrows } from './helpers'

// The privates bridge PROTOTYPE (docs/design-private-bindings.md), acorn
// engine only: `privates: { Db: ['#url'] }` injects a static block into the
// class body — the one place the private brand is legal — publishing
// get/set closures under Symbol.for('wrap-esm-lambda.privates'). These
// tests drive the injection end to end: the transformed module actually
// evaluates (via data: imports) and the bridge operates on genuine branded
// instances. No native twin exists yet, so unlike engine-parity.spec.ts
// nothing here diffs the two engines; the shapes pinned here are what the
// native port and the contract-version bump will be held to.

const BRIDGE = Symbol.for(engine.PRIVATE_BRIDGE_KEY)
const ENTRY = { bindings: ['Db'], patchName: 'traceDb', patchFrom: '/abs/apm-patch.mjs', aliasIndex: 0 }

const tapPrivates = (source: string, privates: Record<string, string[]>, bindings = ['Db']) =>
...
test('the bridge grants real get/set on genuine instances', async () => {
  const out = tapPrivates(DB_SOURCE, { Db: ['#url', '#pool'] })
...
test('the bridge is symbol-keyed and invisible to enumerating reflection', async () => {
  const out = tapPrivates(DB_SOURCE, { Db: ['#url'] })
  const mod = await importModule(out.code! + out.snippets)
...
test('the full patch pattern: subclass wrapper reads privates through the inherited bridge', async () => {
  const seen: string[] = []
...
test('slot shapes follow what the language permits: methods and lone accessors lose a side', async () => {
...
`
  const out = tapPrivates(source, { Shape: ['#hidden', '#count', '#bump', '#virtual', '#input'] }, ['Shape'])
  const mod = await importModule(out.code! + out.snippets)
...
test('a class-valued const gets the bridge and the demotion in one rewrite', async () => {
...
`
  const out = tapPrivates(source, { Client: ['#token'] }, ['Client'])
...
test('entries bridging the same class converge on one injected static block', () => {
  const entries = [
  ...
  assert.strictEqual(out.code!.match(/"#url": \{ get/g)!.length, 1, 'overlapping names dedupe')
  assert.ok(out.code!.includes('"#pool": { get'), 'the union of names is bridged')
})

test('without privates the fast path is untouched', () => {
...
test('refusals are loud, validation-shaped, and never impersonate a missing export', () => {
  const noClass = captureThrows(() => tapPrivates(DB_SOURCE, { Missing: ['#x'] }))
  ...
  const noPrivate = captureThrows(() => tapPrivates(DB_SOURCE, { Db: ['#nope'] }))
...
test('the rewrite map still resolves positions across the injection', async () => {
  const out = tapPrivates(DB_SOURCE, { Db: ['#url'] })
  assert.ok(out.map)
