...
test('buffer-input variant: identical to the string transform, UTF-8 in', () => {
  ...
  assert.deepStrictEqual(output, transformLambda(input, 'handler', 'WrapAwsLambda'))
})

test('buffer-input variant: invalid UTF-8 fails loudly', () => {
  const err = captureThrows(() =>
    transformLambdaFromBuffer(Buffer.from([0xff, 0xfe, 0x00]), 'handler', 'WrapAwsLambda'),
  )
...
`
  ...
  assert.ok(output.includes('WrapAwsLambda('))
  ...
  assert.deepStrictEqual(map.sources, ['handler.mjs'])
  assert.deepStrictEqual(map.sourcesContent, [input])
})

test('source map object for chaining', () => {
...
`
  ...
  assert.ok(code.includes('WrapAwsLambda('))
  assert.ok(!code.includes('sourceMappingURL'))
  assert.ok(map)
  const parsed = JSON.parse(map!)
  assert.strictEqual(parsed.version, 3)
  assert.deepStrictEqual(parsed.sources, ['handler.js'])
})

test('chained source map composed in Rust', () => {
...
`
  ...
  const map = JSON.parse(Buffer.from(match![1], 'base64').toString('utf8'))
  assert.strictEqual(map.version, 3)
  assert.deepStrictEqual(map.sources, ['handler.ts'])
  assert.deepStrictEqual(map.sourcesContent, [tsSource])
})
