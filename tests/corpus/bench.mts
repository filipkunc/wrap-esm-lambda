...
//
//   node tests/corpus/bench.mts             # full corpus -> tests/corpus/engines.md
//   node tests/corpus/bench.mts zod rxjs    # a subset (prints, does not write)
...
// ── tables, split by mode ────────────────────────────────────────────────────
// The ENGINE comparison lives in ESM mode only: there the tap parses,
// validates against statically visible exports and (on the rewrite tier)
// regenerates — the work the two engines implement differently. In CJS
// mode the input is IGNORED by design (no parse, no validation; accessors
// are appended sight-unseen), so both engines share the byte pipeline and
// their times tie by construction — the CJS table is a plumbing sanity
// check, not an engine race, and a gap there would be a bug report.
const esmRows = rows.filter((r) => r.format === 'module')
const cjsRows = rows.filter((r) => r.format === 'commonjs')
...
function totalsFor(subset: BenchRow[]): { totalOxc: number; totalAcorn: number; geomean: number } {
  const totalOxc = subset.reduce((sum, r) => sum + r.oxcMs, 0)
  const totalAcorn = subset.reduce((sum, r) => sum + r.acornMs, 0)
  const geomean = Math.exp(subset.reduce((sum, r) => sum + Math.log(r.acornMs / r.oxcMs), 0) / subset.length)
  return { totalOxc, totalAcorn, geomean }
}

function tableFor(subset: BenchRow[]): string[] {
  ...
  const { totalOxc, totalAcorn } = totalsFor(subset)
  out.push(
    `| **total** | ${subset.length} entries | | | | **${fmtMs(totalOxc)}** | **${fmtMs(totalAcorn)}** | **${(totalAcorn / totalOxc).toFixed(1)}×** | |`,
  )
  return out
}

const esm = totalsFor(esmRows)
const cjs = totalsFor(cjsRows)
const styledRewrites = rows.filter((r) => !r.identical && r.tier === 'rewrite').length
const md = [
  '# Engine benchmark on the corpus',
  '',
  '_Generated by `node tests/corpus/bench.mts` — do not edit. Minimum of repeated full-surface identity-tap transforms (the real `applyMatched` pipeline, star walk included) per corpus entry file; one process per engine._',
  '',
  ...
  ...tableFor(cjsRows),
  '',
  `CJS acorn/oxc: geometric mean **${cjs.geomean.toFixed(1)}×** per entry — the expected tie. Append-tier byte-parity (asserted across BOTH tables): ${mismatches.length === 0 ? 'holds on every append entry' : `**${mismatches.length} MISMATCH(ES)** — see below`}.`,
  '',
...
/**
 * The dumbbell dot plot behind engines.md: one row per ESM corpus entry —
 * the mode where the engines actually parse/validate/rewrite; CJS entries
 * tie by construction and live in their own table, not the chart. Two dots
 * (oxc, acorn) per row on a LOG time axis — position encoding, so the log
  ...
  const config = {
    ...
    options: {
      ...
      plugins: {
        ...
        title: {
          display: true,
          text: 'Engine shoot-out on the corpus — ESM entries, where the engines actually parse',
          color: TEXT_PRIMARY,
          font: { size: 14 },
        },
      },
...
console.error(
  `bench done: ESM oxc ${fmtMs(esm.totalOxc)}ms vs acorn ${fmtMs(esm.totalAcorn)}ms (geomean ${esm.geomean.toFixed(1)}×) · CJS geomean ${cjs.geomean.toFixed(1)}× (expected tie)`,
)
